...
st.set_page_config(
    page_title="Wind Turbine SME Assistant | Enterprise", page_icon="⚡", layout="wide"
)
...
with col2:
    prompt = st.text_area(
        "",
        height=120,
        placeholder="Paste the Equipment Logs...",
            ...
            with st.spinner("Analyzing equipment data..."):
                try:
                    response = requests.post(
                        "https://15amive2ah.execute-api.us-east-1.amazonaws.com/Prod/sme_assistant",
                        json={"prompt": prompt},
                    )
                    ...
                    if response.status_code == 200:
                        ...
                        st.markdown(
                            "<div style='margin-top: 24px;'><div style='color: #1e293b; font-weight: 600; margin-bottom: 12px;'>Expert Analysis:</div></div>",
                            unsafe_allow_html=True,
                        )
                        st.markdown(
                            f"<div style='background: #f8fafc; padding: 20px; border-radius: 8px; border: 1px solid #e2e8f0; color: #334155; line-height: 1.6;'>{result}</div>",
                            unsafe_allow_html=True,
                        )
                    else:
                        st.error(
                            f"System Error: Unable to process request (Status: {response.status_code})"
                        )
